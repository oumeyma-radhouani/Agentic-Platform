...
def _generate_macro_insights(processed_records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    # Génération des recommandations logiques
    recos = []
    if top_frictions:
        recos.append(f"Alerte Prioritaire : '{top_frictions[0]['theme']}' est la cause majeure d'insatisfaction. Une action corrective ciblée est requise.")
    ...
    if not recos:
        recos.append("Les données actuelles ne dégagent pas de tendance forte. Maintenir le suivi.")
...
def run_batch(
    ...
    *,
    ...
    # --- AJOUT DE L'INTELLIGENCE STRATÉGIQUE DANS LA RÉPONSE ---
